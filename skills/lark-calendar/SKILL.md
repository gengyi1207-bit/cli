...
默认查询当天。若用户未指定查询时间范围，先确认意图；如果上下文允许直接执行，则明确采用“今天整天”作为默认范围，并传入带时区偏移的 `--start`/`--end` 以保持用户当前时区语义。结果应整理为按日期分组、按开始时间升序的易读时间线。
...
- **日程查询默认范围**：用户查询日程但未指定时间范围时，应先确认；如果上下文允许直接执行，则明确采用“今天整天”作为默认范围，并按用户当前时区的本地自然日生成带时区偏移的 `--start`/`--end`，不依赖 CLI 进程时区，也不静默转换为 UTC 日或其他时区日期。
